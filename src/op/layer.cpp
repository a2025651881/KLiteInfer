...
namespace op{
    BaseLayer::BaseLayer(base::DeviceType device_type,LayerType layer_type,base::DataType data_type,
                        std::string layer_name):device_type_(device_type),layer_name_(layer_name),data_type_(data_type),layer_type_(layer_type){}

    base::LayerType BaseLayer::layer_type() const{
        return layer_type_;
    }
    ...
    void BaseLayer::set_device_type(baseDeviceType device_type){
    ...
    void Layer::set_output(int32_t idx,const tesnor::Tensor output){
        CHECK_GE(idx,0);
        CHECK_LT(idx,output_.size());
        this->output_.at(idx) = output;
    }
    ...
    tensor::tensor& Layer::get_input(int32_t idx) const{
        CHECK_GE(idx,0);
        CHECK_LT(idx,inputs_.size());
        return inputs_.at(idx);
    }

    tensor::tensor& Layer::get_output(int32_t idx) const{
        CHECK_GE(idx,0);
        CHECK_LT(idx,output_.size());
    ...
    void Layer::set_cuda_config(std::shared_ptr<kernel::CudaCpnfig> config){
        if(!config) return;
    ...
    LayerParam::LayerParam(base::DeviceType  device_type,LayerType layer_type,
                            bool is_quant_layer =false,std::string layer_name = ""): Layer(device_type, layer_type, std::move(layer_name)), is_quant_layer_(is_quant_layer){}
    ...
    size_t LayerParam::weight_size() const{return weight_.size()}
    ...
    base::Status forward(const tensor::Tensor& input1,const tensor::Tensor& input2,const Tensor::Tensor& output){
        this->set_input(0,input1);
    ...
    base::Status forward(const tensor::Tensor& input1,const tensor::Tensor& input2,tensor::Tensor& input3,const Tensor::Tensor& output){
        this->set_input(0,input1);
        this->set_input(1,input2);
